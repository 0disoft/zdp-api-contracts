...
export async function loadApiContracts(root = process.cwd()): Promise<ApiContracts> {
  ...
  const [
    ...
    await Promise.all([
      ...
      loadContract(
        contractsRoot,
        'oidc-product-session',
        join('apis', 'core-api', 'oidc-product-session.yaml'),
        parseOidcProductSessionContract
      ),
      loadContract(
        contractsRoot,
        'oidc-client-registry',
        join('apis', 'core-api', 'oidc-client-registry.yaml'),
        parseOidcClientRegistryContract
      ),
      loadContract(
        contractsRoot,
        'oidc-provider-runtime',
        join('apis', 'core-api', 'oidc-provider-runtime.yaml'),
...
export function parseOidcProductSessionContract(
  source: string
): OidcProductSessionContract {
  ...
  return {
    ...
    oauthSecurityBaseline: requiredString(
      contract,
      'oauth_security_baseline',
      context
    ),
    ...
    requiredAuthorizationBindings: requiredStringList(
      contract,
      'required_authorization_bindings',
      context
    ),
    requiredTokenExchangeBindings: requiredStringList(
      contract,
      'required_token_exchange_bindings',
      context
    ),
    requiredClientRegistryFields: requiredStringList(
      contract,
      'required_client_registry_fields',
      context
    ),
    requiredAccessDecisionBindings: requiredStringList(
      contract,
      'required_access_decision_bindings',
      context
    ),
    invalidationEvents: requiredStringList(
      contract,
      'invalidation_events',
      context
    ),
    exactRedirectUriMatchRequired: requiredBoolean(
      contract,
      'exact_redirect_uri_match_required',
      context
    ),
    wildcardRedirectUriForbidden: requiredBoolean(
      contract,
      'wildcard_redirect_uri_forbidden',
      context
    ),
    arbitraryReturnToForbidden: requiredBoolean(
      contract,
      'arbitrary_return_to_forbidden',
      context
    ),
    ...
    authorizationCodeTtlPolicy: requiredString(
      contract,
      'authorization_code_ttl_policy',
      context
    ),
    tokenEndpointCaller: requiredString(
      contract,
      'token_endpoint_caller',
      context
    ),
    browserTokenExposurePolicy: requiredString(
      contract,
      'browser_token_exposure_policy',
      context
    ),
    productCookiePolicy: requiredString(
      contract,
      'product_cookie_policy',
      context
    ),
    productSessionOwner: requiredString(
      contract,
      'product_session_owner',
      context
    ),
    centralSessionOwner: requiredString(
      contract,
      'central_session_owner',
      context
    ),
    authorizationOwner: requiredString(
      contract,
      'authorization_owner',
      context
    ),
    authenticationIsAuthorization: requiredBoolean(
      contract,
      'authentication_is_authorization',
      context
    ),
    clientRegistryPolicy: requiredString(
      contract,
      'client_registry_policy',
      context
    ),
...
function parseOidcClientRegistryLifecycle(
...
function parseOidcClientRegistryEntry(
...
export function parseAccessDecisionContract(
  ...
  return {
    ...
    httpProfile: contract.http_profile === undefined ? null : parseAccessDecisionHttpProfile(
      requiredObject(contract, 'http_profile', context), `${context}.http_profile`
    ),
...
function parseAccessDecisionHttpProfile(
  profile: Record<string, unknown>, context: string
): NonNullable<AccessDecisionContract['httpProfile']> {
