 ...
 */
export function compareOpenApiRouteCatalog(
  source: string,
  file: string,
  routes: readonly ApiRouteDefinition[],
  requestedServiceId: string | null = null
): OpenApiRouteCatalogCheckResult {
  ...
  if (
    declaredServiceIds.root !== null &&
    declaredServiceIds.info !== null &&
    declaredServiceIds.root !== declaredServiceIds.info
  ) {
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_DECLARED_SERVICE_ID_MISMATCH',
        file,
        'info.x-zdp-service-id',
        `OpenAPI root service \`${declaredServiceIds.root}\` does not match info service \`${declaredServiceIds.info}\`.`
      )
    );
  }
  if (serviceId === null) {
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_SERVICE_ID_REQUIRED',
        file,
        'x-zdp-service-id',
        'OpenAPI comparison requires --service or a root/info x-zdp-service-id value.'
      )
    );
  }
  if (
    ...
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_SERVICE_ID_MISMATCH',
        file,
        'x-zdp-service-id',
        `OpenAPI declares service \`${declaredServiceId}\` but the CLI requested \`${requestedServiceId}\`.`
      )
    );
  }
...
function validateOpenApiVersion(
  ...
  if (
    ...
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_VERSION_UNSUPPORTED',
        file,
        'openapi',
        'OpenAPI route comparison supports explicit OpenAPI 3.0.x and 3.1.x documents.'
      )
    );
  }
}

function readDeclaredServiceIds(document: Record<string, unknown>): {
...
function collectOpenApiOperations(
  ...
  for (const [path, pathValue] of Object.entries(paths)) {
    ...
    if (!isRecord(pathValue)) {
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_PATH_ITEM_INVALID',
          file,
          pathContext,
          `OpenAPI path \`${path}\` must declare an object path item.`
        )
      );
      continue;
    }
    if (typeof pathValue.$ref === 'string') {
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_PATH_ITEM_REF_UNSUPPORTED',
          file,
          `${pathContext}.$ref`,
          'Route catalog comparison requires inline path operations instead of a path-item $ref.'
        )
      );
    }
    ...
    for (const method of OPENAPI_OPERATION_METHODS) {
      ...
      if (!isRecord(operationValue)) {
        diagnostics.push(
          createDiagnostic(
            'API_OPENAPI_OPERATION_INVALID',
            file,
            operationContext,
            `OpenAPI operation ${method.toUpperCase()} ${path} must be an object.`
          )
        );
        continue;
      }
      if (typeof operationValue.$ref === 'string') {
        diagnostics.push(
          createDiagnostic(
            'API_OPENAPI_OPERATION_REF_UNSUPPORTED',
            file,
            `${operationContext}.$ref`,
            'Route catalog comparison requires inline operation metadata instead of an operation $ref.'
          )
        );
      }
      ...
      operations.push({
        ...
        successStatuses: readSuccessStatuses(
          operationValue.responses,
          file,
          `${operationContext}.responses`,
          diagnostics
        ),
...
function readSuccessStatuses(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_RESPONSES_MISSING',
        file,
        path,
        'OpenAPI operation must declare a responses object.'
      )
    );
    return [];
  }
  ...
  for (const responseStatus of Object.keys(value)) {
    ...
    if (/^2XX$/i.test(responseStatus)) {
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_SUCCESS_STATUS_RANGE_UNSUPPORTED',
          file,
          `${path}.${responseStatus}`,
          'OpenAPI comparison requires explicit 2xx response statuses instead of a 2XX range.'
        )
      );
    }
  }
...
function validateUniqueOperationIds(
  ...
  const firstOperationById = new Map<string, OpenApiOperation>();
  for (const operation of operations) {
    if (operation.operationId === null) {
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_OPERATION_ID_MISSING',
          file,
          `${operation.diagnosticPath}.operationId`,
          `OpenAPI operation ${operation.method} ${operation.path} must declare operationId.`
        )
      );
      continue;
    }
    ...
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_OPERATION_ID_DUPLICATE',
        file,
        `${operation.diagnosticPath}.operationId`,
        `OpenAPI operationId \`${operation.operationId}\` is already used by ${first.method} ${first.path}.`
      )
    );
  }
}
...
function compareServiceRoutes(
  ...
  if (serviceRoutes.length === 0) {
    diagnostics.push(
      createDiagnostic(
        'API_OPENAPI_SERVICE_NOT_FOUND',
        'contracts/apis/catalog.yaml',
        'routes',
        `Route catalog has no entries for service \`${serviceId}\`.`
      )
    );
    return;
  }

  const catalogRouteByKey = new Map(
    serviceRoutes.map((route) => [routeKey(route.method, route.path), route])
  );
  const openApiOperationByKey = new Map(
  ...
  for (const route of serviceRoutes) {
    ...
    if (operation === undefined) {
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_OPERATION_MISSING',
          file,
          operationPath,
          `OpenAPI is missing catalog route ${route.method} ${route.path} (${route.operationId}).`
        )
      );
      continue;
    }
    if (
      ...
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_OPERATION_ID_MISMATCH',
          file,
          `${operation.diagnosticPath}.operationId`,
          `OpenAPI operationId \`${operation.operationId ?? '<missing>'}\` must equal catalog operationId \`${route.operationId}\`.`
        )
      );
    }
    for (const status of route.successStatuses) {
      if (!operation.successStatuses.includes(status)) {
        diagnostics.push(
          createDiagnostic(
            'API_OPENAPI_SUCCESS_STATUS_MISSING',
            file,
            `${operation.diagnosticPath}.responses`,
            `OpenAPI operation ${route.operationId} is missing catalog success status ${status}.`
          )
        );
      }
    }
    for (const status of operation.successStatuses) {
      if (!route.successStatuses.includes(status)) {
        diagnostics.push(
          createDiagnostic(
            'API_OPENAPI_SUCCESS_STATUS_UNDECLARED',
            file,
            `${operation.diagnosticPath}.responses.${status}`,
            `OpenAPI operation ${route.operationId} declares 2xx status ${status} outside the route catalog.`
          )
        );
      }
    }
  ...
  for (const operation of operations) {
    if (!catalogRouteByKey.has(routeKey(operation.method, operation.path))) {
      diagnostics.push(
        createDiagnostic(
          'API_OPENAPI_ROUTE_UNDECLARED',
          file,
          operation.diagnosticPath,
          `OpenAPI operation ${operation.method} ${operation.path} is not declared for service \`${serviceId}\` in the route catalog.`
        )
      );
    }
  }
