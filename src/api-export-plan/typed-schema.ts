...
export type ApiDeclaredSchemaPropertyType =
  (typeof DECLARED_PROPERTY_TYPES)[number];
 ...
 */
export async function loadTypedSchemaRegistry(
  root: string,
  bundles: readonly ApiSchemaBundleContract[]
): Promise<ApiTypedSchemaRegistryResult> {
  const resolvedRoot = resolve(root);
  const results = await Promise.all(
    bundles.map(async (bundle): Promise<ApiTypedSchemaBundleParseResult> => {
      ...
      if (
        ...
        return failedBundle(
          bundle.file,
          'API_TYPED_SCHEMA_PATH_ESCAPE',
          'schema_bundle',
          `Typed schema source \`${bundle.file}\` must remain under the package root.`
        );
      }
  ...
  const entries = results
    ...
    .map((schema) => [schema.schemaRef, schema] as const)
...
export function parseTypedSchemaBundle(
  source: string,
  bundle: ApiSchemaBundleContract
): ApiTypedSchemaBundleParseResult {
  let parsed: unknown;
  try {
  ...
  if (!isRecord(parsed)) {
    return failedBundle(
      bundle.file,
      'API_TYPED_SCHEMA_BUNDLE_INVALID',
      'schema_bundle',
      'Schema bundle YAML must parse to an object.'
    );
  }
  ...
  if (!isRecord(rawBundle) || !Array.isArray(rawBundle.schemas)) {
    return failedBundle(
      ...
      'Schema bundle must declare a schemas array.'
    );
  }

  const diagnostics: ApiContractDiagnostic[] = [];
  const rawSchemasById = new Map<string, Record<string, unknown>>();
  for (const [index, rawSchema] of rawBundle.schemas.entries()) {
    if (!isRecord(rawSchema) || typeof rawSchema.id !== 'string') {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_DEFINITION_INVALID',
          bundle.file,
          `schema_bundle.schemas[${index}]`,
          'Typed schema metadata requires a string schema id.'
        )
      );
      continue;
    }
    if (rawSchemasById.has(rawSchema.id)) {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_ID_DUPLICATE',
          bundle.file,
          `schema_bundle.schemas[${index}].id`,
          `Schema id \`${rawSchema.id}\` is duplicated.`
        )
      );
      continue;
    }
  ...
  const schemas = bundle.schemas.map((schema) => {
    ...
    if (rawSchema === undefined) {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_DEFINITION_MISSING',
          bundle.file,
          `schema_bundle.schemas[${schema.id}]`,
          `Parsed schema \`${schema.id}\` is missing from its source bundle.`
        )
      );
...
function parseSchemaDefinition(
  ...
  if (!isRecord(propertiesValue)) {
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_PROPERTIES_INVALID',
        bundle.file,
        path,
        'Typed schema properties must be an object keyed by field name.'
      )
    );
    return buildLegacySchema(bundle, schema);
  }
  ...
  for (const secretField of schema.secretFields) {
    if (!declaredFields.includes(secretField)) {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_SECRET_FIELD_UNDECLARED',
          bundle.file,
          `schema_bundle.schemas[${schema.id}].secret_fields`,
          `Secret field \`${secretField}\` must also be required or optional.`
        )
      );
    }
  }
    ...
    if (!Object.hasOwn(propertiesValue, field)) {
      diagnostics.push(
        diagnostic(
          ...
          bundle.file,
          path,
          `Typed schema \`${schema.id}\` must define property \`${field}\`.`
        )
      );
    }
  }
    ...
    if (!declaredFields.includes(field)) {
      diagnostics.push(
        diagnostic(
          ...
          bundle.file,
          `${path}.${field}`,
          `Typed property \`${field}\` must be listed in required_fields or optional_fields.`
        )
      );
    }
  }
...
function parseProperty(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_PROPERTY_INVALID',
        file,
        path,
        'Schema property must be an object.'
      )
    );
    return unknownProperty();
  }
  for (const key of Object.keys(value)) {
    if (!PROPERTY_KEYS.includes(key as (typeof PROPERTY_KEYS)[number])) {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_PROPERTY_KEY_UNKNOWN',
          file,
          `${path}.${key}`,
          `Schema property must not declare unknown key \`${key}\`.`
        )
      );
    }
  }
  ...
  if (
    ...
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_FORMAT_TYPE_INVALID',
        file,
        `${path}.format`,
        'format is only valid for string, integer, or number properties.'
      )
    );
  }
  const nullable = readOptionalBoolean(
    value.nullable,
    false,
    file,
    `${path}.nullable`,
  ...
  let items: ApiTypedSchemaProperty | null = null;
  if (type === 'array') {
    if (!Object.hasOwn(value, 'items')) {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_ARRAY_ITEMS_MISSING',
          file,
          `${path}.items`,
          'Array schema property must declare items.'
        )
      );
    ...
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_ITEMS_FORBIDDEN',
        file,
        `${path}.items`,
        'Only array schema properties may declare items.'
      )
    );
  }

  let properties: Readonly<Record<string, ApiTypedSchemaProperty>> = {};
  let requiredProperties: readonly string[] = [];
  ...
  if (type === 'object') {
    ...
    for (const field of requiredProperties) {
      if (!Object.hasOwn(properties, field)) {
        diagnostics.push(
          diagnostic(
            'API_TYPED_SCHEMA_OBJECT_REQUIRED_FIELD_MISSING',
            file,
            `${path}.required`,
            `Object required field \`${field}\` must exist in properties.`
          )
        );
      }
    }
    additionalProperties = readOptionalBoolean(
    ...
    for (const key of ['properties', 'required', 'additional_properties'] as const) {
      if (Object.hasOwn(value, key)) {
        diagnostics.push(
          diagnostic(
            'API_TYPED_SCHEMA_OBJECT_KEY_FORBIDDEN',
            file,
            `${path}.${key}`,
            `Only object schema properties may declare ${key}.`
          )
        );
      }
    }
...
function parseNestedProperties(
  ...
  if (!isRecord(value)) {
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_OBJECT_PROPERTIES_INVALID',
        file,
        path,
        'Object properties must be an object when set.'
      )
    );
    return {};
  }
...
function parseEnumValues(
  value: unknown,
  type: ApiDeclaredSchemaPropertyType,
  nullable: boolean,
  file: string,
  path: string,
  diagnostics: ApiContractDiagnostic[]
): readonly ApiSchemaEnumValue[] {
  if (value === undefined) {
    return [];
  }
  if (!Array.isArray(value) || value.length === 0) {
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_ENUM_INVALID',
        file,
        path,
        'enum must be a non-empty scalar array.'
      )
    );
    return [];
  }
  if (type === 'array' || type === 'object') {
    diagnostics.push(
      diagnostic(
        'API_TYPED_SCHEMA_ENUM_TYPE_INVALID',
        file,
        path,
        'enum is only supported for scalar properties.'
      )
    );
    return [];
  }

  const values: ApiSchemaEnumValue[] = [];
  for (const item of value) {
    if (!isEnumValue(item) || !enumMatchesType(item, type, nullable)) {
      diagnostics.push(
        diagnostic(
          'API_TYPED_SCHEMA_ENUM_VALUE_INVALID',
          file,
          path,
          `enum value \`${String(item)}\` does not match ${type}${nullable ? ' or null' : ''}.`
        )
      );
      continue;
    }
...
function enumMatchesType(
  value: ApiSchemaEnumValue,
  type: ApiDeclaredSchemaPropertyType,
  nullable: boolean
): boolean {
  if (value === null) {
...
function buildLegacySchema(
  ...
  return {
    ...
    typed: false,
    properties: Object.fromEntries(
      fields.map((field) => [field, unknownProperty()])
    )
  };
}

function unknownProperty(): ApiTypedSchemaProperty {
  return {
    type: 'unknown',
    format: null,
    enumValues: [],
...
function readDeclaredType(
  ...
  path: string,
  diagnostics: ApiContractDiagnostic[]
): ApiDeclaredSchemaPropertyType | null {
  if (
    typeof value === 'string' &&
    DECLARED_PROPERTY_TYPES.includes(
      value as ApiDeclaredSchemaPropertyType
    )
  ) {
    return value as ApiDeclaredSchemaPropertyType;
  }
  diagnostics.push(
    diagnostic(
      'API_TYPED_SCHEMA_PROPERTY_TYPE_INVALID',
      file,
      path,
      `Schema property type must be one of ${DECLARED_PROPERTY_TYPES.join(', ')}.`
    )
  );
  return null;
}
...
function readOptionalString(
  ...
  diagnostics.push(
    diagnostic(
      'API_TYPED_SCHEMA_STRING_INVALID',
      file,
      path,
      'Expected a non-empty string when set.'
    )
  );
  return null;
}
...
function readOptionalBoolean(
  ...
  diagnostics.push(
    diagnostic(
      'API_TYPED_SCHEMA_BOOLEAN_INVALID',
      file,
      path,
      'Expected a boolean when set.'
    )
  );
  return fallback;
}
...
function readOptionalStringList(
  ...
  diagnostics.push(
    diagnostic(
      'API_TYPED_SCHEMA_STRING_LIST_INVALID',
      file,
      path,
      'Expected a string array when set.'
    )
  );
  return [];
}
...
function isEnumValue(value: unknown): value is ApiSchemaEnumValue {
  return (
    value === null ||
    typeof value === 'string' ||
