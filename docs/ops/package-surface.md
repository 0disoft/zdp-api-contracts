...
- CLI source: `src/api-contracts/cli-bin.ts`; npm bin: `zdp-api-contracts`
...
Package files whitelist는 빌드된 `dist/`, 계약 원문과 운영 문서만 포함한다. TypeScript source, generated OpenAPI, generated SDK, live endpoint 정보, customer payload fixture는 포함하지 않는다. 공개 parser와 OpenAPI 대조 CLI의 `yaml` 의존성은 package runtime dependency로 명시한다. npm publish는 `prepack`에서 `dist/`를 재생성하고, commit SHA로 고정한 Git dependency는 검증 후 커밋된 같은 `dist/`를 소비한다. Tarball smoke는 빈 Node 소비자에서 실제 import, 계약 로딩, package bin map과 JSON CLI 실행을 확인한다.

CLI는 계약 검증 결과를 text, JSON 또는 SARIF로 출력하고, 선택적으로 한 서비스 OpenAPI의 method, path, operation ID와 명시적 2xx status를 route catalog와 대조한다. OpenAPI artifact 생성, schema publish, handler 실행과 live endpoint 판정은 수행하지 않는다. 사용법은 `contract-cli.md`를 따른다.
