import { parseAbuseChallengeContract, parseApiCatalogContract, parseErrorEnvelopeContract, parseProductLinkHandoffContract, parseRouteContract, parseSdkGenerationInputContract, parseWebhookContract } from './strict-parser.js';
import { parseAccessDecisionContract, parseCalculatorCatalogContract, parseCalculatorConformanceContract, parseCreditPurchaseContract, parseCustomerPolicyRegistryContract, parseOidcClientRegistryContract, parseOidcProductSessionContract, parseOidcProviderRuntimeContract, parseSensitiveActionAuthorizationContract } from './parser.js';
export function defineApiContractFamily(registration) {
    return registration;
}
...
export function apiContractFamilySourcePath(key) {
    const registration = API_CONTRACT_FAMILY_REGISTRY.find((candidate) => candidate.key === key);
    if (registration === undefined) {
        throw new Error(`API contract family \`${key}\` is not registered.`);
    }
    return registration.sourcePath;
}
export function apiContractFamilySourcesForExport(target) {
...
export function listApiSchemaBundleSourcePaths(catalog) {
    return uniqueSorted([
...
export function validateApiContractFamilyRegistry(registrations = API_CONTRACT_FAMILY_REGISTRY) {
    const diagnostics = [];
    ...
    for (const [index, registration] of registrations.entries()) {
        ...
        if (expectedKey !== undefined && registration.key !== expectedKey) {
            diagnostics.push({
                ...
                path: `${path}.key`,
                message: `API contract family position ${index} must register ` +
        ...
        if (!/^[a-z][a-z0-9-]*$/.test(registration.name)) {
            diagnostics.push({
                ...
                file: 'src/api-contracts/family-registry.ts',
                path: `${path}.name`,
                message: `API contract family name \`${registration.name}\` must be a stable ` +
        ...
        if (!isContractSourcePath(registration.sourcePath)) {
            diagnostics.push({
                ...
                file: 'src/api-contracts/family-registry.ts',
                path: `${path}.sourcePath`,
                message: `API contract source \`${registration.sourcePath}\` must remain under ` +
...
function schemaBundleSourcePathFromRef(schemaRef) {
...
function isContractSourcePath(sourcePath) {
