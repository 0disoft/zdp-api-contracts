...
import { ApiContractLoadError, loadApiContracts } from './parser.js';
import { validateApiContracts } from './validator.js';
import { compareOpenApiRouteCatalog } from './openapi-route-catalog.js';
import { renderApiContractCheckReport } from './cli-report.js';
...
export async function runApiContractCheckCli(argv, runtime = defaultRuntime()) {
    let options;
    try {
    ...
    try {
        ...
        if (options.openApi !== null) {
            ...
            const comparison = await compareOpenApiFile(openApiFile, displayFile, contracts.apiCatalog.routes, options.serviceId);
    ...
    try {
        ...
        runtime.writeStderr(`API contract report write failed: ${error instanceof Error ? error.message : String(error)}\n`);
        return 1;
    }
...
function splitInlineOption(argument) {
...
function parseFormat(value) {
    ...
    throw new CliUsageError(`--format must be text, json, or sarif; received ${value}.`);
}
function setFormat(format, formatWasSet) {
    if (formatWasSet) {
        throw new CliUsageError('Output format may be selected only once.');
    }
    return format;
}
async function compareOpenApiFile(openApiFile, displayFile, routes, serviceId) {
    try {
        const source = await readFile(openApiFile, 'utf8');
        return compareOpenApiRouteCatalog(source, displayFile, routes, serviceId);
    }
    catch (error) {
        return {
            ok: false,
...
function toOpenApiComparisonReport(file, comparison) {
    return {
        file,
...
function contractLoadDiagnostics(error) {
...
async function emitReport(report, options, runtime) {
