...
const SCHEMA_BUNDLE_COMPANION_ROOT_BY_FILE = {
    ...
    'contracts/apis/core-api/customer-policy-registry.yaml': 'customer_policy_registry',
    'contracts/apis/core-api/product-link.yaml': 'product_link_handoff',
    'contracts/apis/core-api/sensitive-action-authorization.yaml': 'sensitive_action_authorization',
...
export async function loadApiContracts(root = process.cwd()) {
    ...
    for (const schemaBundle of contracts.schemaBundles) {
        if (!assertions.has(schemaBundle.file)) {
            assertions.set(schemaBundle.file, (source) => assertApiSchemaBundleSource(source, schemaBundle.file));
        }
    }
    await Promise.all(Array.from(assertions, ([file, assertSourceShape]) => readAndAssertSource(root, file, assertSourceShape)));
    return contracts;
}
...
async function readAndAssertSource(root, file, assertSourceShape) {
...
function assertAbuseChallengeSource(source) {
    const data = parseYamlObject(source, ABUSE_CHALLENGE_FILE);
    assertOnlyKeys(data, ['abuse_challenge', 'schema_bundle'], ABUSE_CHALLENGE_FILE);
    const contract = requiredObject(data, 'abuse_challenge', ABUSE_CHALLENGE_FILE);
    assertOnlyKeys(contract, ABUSE_CHALLENGE_KEYS, `${ABUSE_CHALLENGE_FILE}#abuse_challenge`);
    assertApiSchemaBundleObject(data, ABUSE_CHALLENGE_FILE);
}
function assertProductLinkSource(source) {
    const data = parseYamlObject(source, PRODUCT_LINK_FILE);
    assertOnlyKeys(data, ['product_link_handoff', 'schema_bundle'], PRODUCT_LINK_FILE);
    const contract = requiredObject(data, 'product_link_handoff', PRODUCT_LINK_FILE);
    const context = `${PRODUCT_LINK_FILE}#product_link_handoff`;
    assertOnlyKeys(contract, PRODUCT_LINK_KEYS, context);
    requiredRecordListAllowEmpty(contract, 'allowed_transitions', context).forEach((transition, index) => assertOnlyKeys(transition, ['from', 'event', 'to'], `${context}.allowed_transitions[${index}]`));
    assertApiSchemaBundleObject(data, PRODUCT_LINK_FILE);
}
function assertRouteContractSource(source) {
    const data = parseYamlObject(source, ROUTE_CONTRACT_FILE);
    assertOnlyKeys(data, ['route_contract'], ROUTE_CONTRACT_FILE);
    assertOnlyKeys(requiredObject(data, 'route_contract', ROUTE_CONTRACT_FILE), ROUTE_CONTRACT_KEYS, `${ROUTE_CONTRACT_FILE}#route_contract`);
}
function assertErrorEnvelopeSource(source) {
    const data = parseYamlObject(source, ERROR_ENVELOPE_FILE);
    assertOnlyKeys(data, ['error_envelope'], ERROR_ENVELOPE_FILE);
    assertOnlyKeys(requiredObject(data, 'error_envelope', ERROR_ENVELOPE_FILE), ERROR_ENVELOPE_KEYS, `${ERROR_ENVELOPE_FILE}#error_envelope`);
}
function assertWebhookContractSource(source) {
    const data = parseYamlObject(source, WEBHOOK_CONTRACT_FILE);
    assertOnlyKeys(data, ['webhook_contract'], WEBHOOK_CONTRACT_FILE);
    assertOnlyKeys(requiredObject(data, 'webhook_contract', WEBHOOK_CONTRACT_FILE), WEBHOOK_CONTRACT_KEYS, `${WEBHOOK_CONTRACT_FILE}#webhook_contract`);
}
function assertSdkGenerationInputSource(source) {
    ...
    assertOnlyKeys(requiredObject(data, 'sdk_generation_input', SDK_GENERATION_INPUT_FILE), SDK_GENERATION_INPUT_KEYS, `${SDK_GENERATION_INPUT_FILE}#sdk_generation_input`);
}
function assertApiCatalogSource(source) {
    const data = parseYamlObject(source, API_CATALOG_FILE);
    assertOnlyKeys(data, ['api_catalog', 'routes'], API_CATALOG_FILE);
    assertOnlyKeys(requiredObject(data, 'api_catalog', API_CATALOG_FILE), API_CATALOG_KEYS, `${API_CATALOG_FILE}#api_catalog`);
    requiredRecordListAllowEmpty(data, 'routes', API_CATALOG_FILE).forEach((route, index) => assertOnlyKeys(route, API_ROUTE_KEYS, `${API_CATALOG_FILE}#routes[${index}]`));
}
function assertApiSchemaBundleSource(source, file) {
    const data = parseYamlObject(source, file);
    const companionRoot = SCHEMA_BUNDLE_COMPANION_ROOT_BY_FILE[file];
    assertOnlyKeys(data, companionRoot === undefined
        ? ['schema_bundle']
        : ['schema_bundle', companionRoot], file);
    assertApiSchemaBundleObject(data, file);
}
function assertApiSchemaBundleObject(data, file) {
    ...
    assertOnlyKeys(requiredObject(schemaBundle, 'common_envelope', schemaBundleContext), API_SCHEMA_COMMON_ENVELOPE_KEYS, commonEnvelopeContext);
    requiredRecordListAllowEmpty(schemaBundle, 'schemas', schemaBundleContext).forEach((schema, index) => assertOnlyKeys(schema, API_SCHEMA_DEFINITION_KEYS, `${schemaBundleContext}.schemas[${index}]`));
}
...
//# sourceMappingURL=strict-parser.js.map