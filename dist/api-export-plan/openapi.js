...
export async function buildOpenApi31Document(root = process.cwd(), options = {}) {
    ...
    try {
        ...
        return failedBuild([
            diagnostic('API_OPENAPI_CONTRACT_LOAD_FAILED', 'contracts', 'root', error instanceof Error ? error.message : String(error))
        ]);
    }
    ...
    const typedRegistryResult = await loadTypedSchemaRegistry(root, contracts.schemaBundles);
    ...
    if (options.strictTypedSchemas === true &&
        registry.untypedSchemaRefs.length > 0) {
        return {
            ok: false,
            document: null,
            diagnostics: registry.untypedSchemaRefs.map((schemaRef) => diagnostic('API_OPENAPI_TYPED_SCHEMA_REQUIRED', schemaFile(schemaRef), `schema_bundle.schemas[${schemaId(schemaRef)}].properties`, `OpenAPI strict mode requires typed properties for \`${schemaRef}\`.`)),
    ...
    try {
        ...
        return {
            ...failedBuild([
                diagnostic('API_OPENAPI_PACKAGE_MANIFEST_INVALID', 'package.json', 'package', error instanceof Error ? error.message : String(error))
            ]),
            typedSchemaRefs: registry.typedSchemaRefs,
            untypedSchemaRefs: registry.untypedSchemaRefs
        };
    ...
    const routes = contracts.apiCatalog.routes
        .filter((route) => includeRestrictedRoutes ||
    ...
    return {
        ...
        document: {
            ...
            'x-zdp-typed-schema-coverage': {
                ...
                total: registry.typedSchemaRefs.length + registry.untypedSchemaRefs.length
            },
...
export function serializeOpenApi31Document(document) {
    return `${JSON.stringify(sortJsonValue(document), null, 2)}\n`;
}
function buildSchemaContexts(contracts, registry) {
    const diagnostics = [];
    ...
    for (const bundle of contracts.schemaBundles) {
        for (const schema of bundle.schemas) {
            ...
            if (typed === undefined) {
                diagnostics.push(diagnostic('API_OPENAPI_TYPED_SCHEMA_LOOKUP_FAILED', bundle.file, `schema_bundle.schemas[${schema.id}]`, `Typed schema registry is missing \`${schemaRef}\`.`));
                continue;
            }
            ...
            if (existingOwner !== undefined && existingOwner !== schemaRef) {
                diagnostics.push(diagnostic('API_OPENAPI_COMPONENT_NAME_CONFLICT', bundle.file, `schema_bundle.schemas[${schema.id}].id`, `OpenAPI component name \`${componentName}\` is shared by \`${existingOwner}\` and \`${schemaRef}\`.`));
                continue;
            }
            if (componentName === ERROR_COMPONENT_NAME) {
                diagnostics.push(diagnostic('API_OPENAPI_COMPONENT_NAME_RESERVED', bundle.file, `schema_bundle.schemas[${schema.id}].id`, `Schema id \`${ERROR_COMPONENT_NAME}\` is reserved for the standard error envelope.`));
                continue;
            }
    ...
    return {
        contexts: Object.fromEntries(entries.sort(([left], [right]) => left.localeCompare(right))),
        diagnostics
    };
}
function buildSchemaComponents(contexts, contracts) {
...
function buildSchemaComponent(context) {
    const properties = Object.fromEntries(Object.entries(context.typed.properties)
...
function propertyToOpenApiSchema(property) {
    ...
    if (property.type === 'object') {
        schema.properties = Object.fromEntries(Object.entries(property.properties)
            .sort(([left], [right]) => left.localeCompare(right))
            .map(([field, nested]) => [
            field,
...
function buildErrorEnvelopeSchema(contracts) {
    ...
    return {
        type: 'object',
        additionalProperties: false,
        properties: Object.fromEntries(fields.map((field) => [field, knownProperties[field] ?? {}])),
...
function buildOperation(route, contexts) {
    ...
    const responses = Object.fromEntries(route.successStatuses.map((status) => [
        String(status),
...
function buildSuccessResponse(route, status, contexts) {
    const response = {
...
function requireContext(contexts, schemaRef) {
...
function successDescription(status) {
    if (status === 200) {
        return 'OK';
    }
    if (status === 201) {
...
async function loadPackageManifest(root) {
...
function failedBuild(diagnostics) {
    return {
        ok: false,
        document: null,
        diagnostics,
