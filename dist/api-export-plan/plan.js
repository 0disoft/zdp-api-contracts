...
const TRACE_FIELDS = ['request_id', 'trace_id'];
const SDK_EXPORT_ERROR_DETAIL_FIELDS = ['details'];
...
export function buildApiExportPlan(contracts) {
    ...
    const exportableRoutes = contracts.apiCatalog.routes.filter((route) => route.exportPolicy === null || route.exportPolicy === undefined);
    const plan = {
        status: 'plan-only',
...
function buildSchemaModelMap(schemaBundles) {
    const schemaModelEntries = schemaBundles.flatMap((bundle) => bundle.schemas.map((schema) => {
        const model = buildSchemaModel(bundle, schema);
        return [model.schemaRef, model];
    }));
    return Object.fromEntries(schemaModelEntries.sort(([left], [right]) => left.localeCompare(right)));
}
function buildSchemaModel(bundle, schema) {
    return {
...
function requireSchemaModelKind(schema) {
    if (schema.kind === 'request' || schema.kind === 'response') {
        return schema.kind;
    }
    throw new Error(`Validated API schema \`${schema.id}\` has unsupported kind \`${schema.kind}\`.`);
}
function validateExportPlanInputs(contracts) {
    return [
        ...
        ...validateRequiredEntries({
            ...
            required: [
                ...contracts.errorEnvelope.requiredFields,
                ...contracts.errorEnvelope.optionalFields.filter((field) => !SDK_EXPORT_ERROR_DETAIL_FIELDS.includes(field))
            ],
