...
export async function runApiExportPlanCli(argv) {
    ...
    try {
        ...
        console.log(`API export plan: ${result.plan?.outputs.length ?? 0} output(s), ${result.plan?.sdkTargets.length ?? 0} SDK target(s)`);
        for (const output of result.plan?.outputs ?? []) {
            console.log(`- ${output.kind}: ${output.sourceContracts.join(', ')}`);
        }
        return 0;
    }
    catch (error) {
